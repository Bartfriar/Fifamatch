...
rank_position:any;
  constructor(public navCtrl: NavController,
    public afAuth: AngularFireAuth, public navParams: NavParams, public afdb: AngularFireDatabase) {
    this.challengekey = navParams.get('chall_key');
    // this.Seconduser = navParams.get('opponent_name');
    firebase.database().ref('Games').orderByChild('GameID').equalTo(this.challengekey).on("child_added", 
    res=>{
      ...
      firebase.database().ref("users").orderByChild("email").equalTo(this.useremail).on("child_added", 
      usern=>{
...
console.log(this.Displayname)
        firebase.database().ref('LeaderBoards').orderByChild('User').equalTo(this.Displayname).on("child_added", 
        rate=>{
          this.Points = rate.val().Points
          this.Won_game = rate.val().Wins
          this.Lost_game = rate.val().Losses
      });
      firebase.database().ref('users').orderByChild('username').equalTo(this.Displayname).on("child_added", 
      takid=>{
        ...
        {
          // firebase.database().ref('LeaderBoards').orderByChild('User').equalTo(this.Displayname).on("child_added", rate=>{
          //   this.Points = rate.val().Points
            this.Points -= 2;
            this.Lost_game +=1;
            if(this.Points < 100)
        ...
        console.log(this.Won_game);
        firebase.database().ref(`LeaderBoards/${this.userid}`).update({
  ...
  firebase.database().ref('users').orderByChild('username').equalTo(this.Displayname).on("child_added", takid=>{
    this.userid = takid.val().UserID
    console.log(this.userid)
  })
    ...
    if(this.Result == "Winner"){
        ...
        this.Points = this.Points + this.une;
        this.Won_game = this.Won_game + 1;
        console.log(this.Points)
        if(this.Points < 100){
          this.Position = "Beginner"
        }
    ...
    else
    {
      ...
      this.Points = this.Points - 2;
        this.Lost_game = this.Lost_game + 1;
        if(this.Points < 100)
        {
          this.Position = "Beginner"
        }
  ...
  {
    console.log("clicked")
    firebase.database().ref('LeaderBoards').orderByChild('User').equalTo(this.currentuser).on("child_added", ranked=>{
