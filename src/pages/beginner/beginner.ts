...
username:any;
  constructor(public navCtrl: NavController, public navParams: NavParams, public afDb: AngularFireDatabase, 
    public afauth:AngularFireAuth) {
      this.afDb.list('LeaderBoards').valueChanges().subscribe((beginner:any)=>{
      ...
      firebase.database().ref("users").orderByChild("UserID").equalTo(this.currentuid).on("child_added", usern=>{
        this.username = usern.val().username;
        console.log(this.username);
      })
      firebase.database().ref(`LeaderBoards`).orderByChild('Rank').equalTo("Beginner").on("child_added",beg=>{
